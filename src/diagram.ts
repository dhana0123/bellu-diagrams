...
export class Diagram {
   ...
   * func takes in a diagram and returns a diagram
   */
  public apply_to_tagged_recursive(
    tags: string | string[],
  ...
  private update_style(
    stylename: keyof Diagram["style"],
    stylevalue: string | LinearGradient,
    excludedType?: DiagramType[]
  ): Diagram {
    let newd: Diagram = this.copy_if_not_mutable();
    if (excludedType?.includes(newd.type)) {
      ...
      for (let i = 0; i < newd.children.length; i++)
        newd.children[i] = newd.children[i].update_style(
          stylename,
          stylevalue,
          excludedType
        );
  ...
  private update_textdata(
    textdataname: keyof Diagram["textdata"],
    textdatavalue: string
  ): Diagram {
    let newd: Diagram = this.copy_if_not_mutable();
    if (
      newd.type == DiagramType.Text ||
      ...
      for (let i = 0; i < newd.children.length; i++)
        newd.children[i] = newd.children[i].update_textdata(
          textdataname,
          textdatavalue
        );
    } else if (
  ...
  public bounding_box(): [Vector2, Vector2] {
    ...
    if (this.type == DiagramType.Diagram) {
      ...
      const bbox = [new Vector2(minx, miny), new Vector2(maxx, maxy)] as [
        Vector2,
        Vector2
      ];
      ...
      this.type == DiagramType.Text ||
      this.type == DiagramType.MultilineText
    ) {
      const bbox = [this.origin.copy(), this.origin.copy()] as [
        Vector2,
        Vector2
      ];
      this._bbox_cache = bbox;
  ...
  public translate(v: Vector2): Diagram {
    ...
    if (prev_cached_bbox != undefined) {
      newd._bbox_cache = prev_cached_bbox.map((p) =>
        Transform.translate(v)(p)
      ) as [Vector2, Vector2];
    }
    return newd;
  }
  ...
  public rotate(
    angle: number,
    pivot: Vector2 | undefined = undefined
  ): Diagram {
    if (pivot == undefined) {
      pivot = this.origin;
    }
      ...
      switch (d.type) {
        case DiagramType.Text: {
          let fontsize = parseFloat(
            d.textdata["font-size"] || DEFAULT_FONTSIZE
          );
  ...
  public debug(show_index: boolean = true): Diagram {
    ...
    if (this.type == DiagramType.Diagram) {
      ...
      let f_obj =
        this.type == DiagramType.Polygon || DiagramType.Image ? polygon : curve;
      ...
      for (let i = 0; i < points.length; i++) {
        ...
        point_texts.push(
          text(i.toString()).position(points[i]).apply(style_index)
        );
        prev_point = points[i];
      }
...
export function multiline(
  spans: ([string] | [string, Partial<TextData>])[]
): Diagram {
...
export function multiline_bb(
  bbstr: string,
  linespace?: string,
  split_by_word: boolean = false
): Diagram {
  let tspans: TextSpanData[] = BB_multiline.from_BBCode(
    bbstr,
    linespace
  ) as TextSpanData[];

...
const defaultFormat_f: formatFunction = (
  name: string,
  val: any,
  prec?: number
) => {
  let val_str =
    typeof val == "number" && prec != undefined
...
export class Interactive {
  ...
  public draw_function: (
    inp_object: inpVariables_t,
    setter_object?: inpSetter_t
  ) => any = (_) => {};
  ...
  public label(
    ...
    labeldiv.innerHTML = display_format_func(
      variable_name,
      value,
      this.display_precision
    );
  ...
  get_svg_element(
    ...
    if (svg_element == undefined) {
      svg_element = document.createElementNS(
  ...
  public locator(
    ...
    let control_svg = this.get_svg_element(
    ...
    if (this.locatorHandler == undefined) {
      ...
      this.registerEventListener(eventTarget, "mousemove", (evt: any) => {
        locatorHandler.drag(evt);
      });
      this.registerEventListener(eventTarget, "mouseup", (evt: any) => {
        locatorHandler.endDrag(evt);
      });
      this.registerEventListener(eventTarget, "touchmove", (evt: any) => {
        locatorHandler.drag(evt);
      });
      this.registerEventListener(eventTarget, "touchend", (evt: any) => {
        locatorHandler.endDrag(evt);
      });
      this.registerEventListener(eventTarget, "touchcancel", (evt: any) => {
        locatorHandler.endDrag(evt);
      });
    }
    ...
    let locator_svg = this.locatorHandler.create_locator_circle_pointer_svg(
      variable_name,
      radius,
      value,
      color,
    ...
    if (blink) {
      ...
      let blinking_outers = locator_svg.getElementsByClassName(
        "diagramatics-locator-blink"
      );
      for (let i = 0; i < blinking_outers.length; i++)
        (this.locatorHandler as LocatorHandler).addBlinkingCircleOuter(
          blinking_outers[i]
        );
    }
    ...
    if (track_diagram) {
      if (
        track_diagram.type != DiagramType.Polygon &&
  ...
  public locator_custom(
    ...
    let locator_svg = this.locatorHandler!.create_locator_diagram_svg(
      variable_name,
      diagram,
      blink
    );
  ...
  public dnd_draggable_to_container(
    ...
    this.dragAndDropHandler.add_draggable_to_container(
      name,
      diagram,
      container_name
    );
    ...
    if (onclickstart_callback)
      this.dragAndDropHandler.register_clickstart_callback(
        name,
        onclickstart_callback
      );
  }
  ...
  public dnd_draggable(
    ...
    this.inp_variables[name] = diagram.origin;
    this.dragAndDropHandler.add_draggable_with_container(
      name,
      diagram,
      container_diagram
    );
   ...
   */
  public dnd_register_move_validation_function(
  ...
  public dnd_move_to_container(name: string, container_name: string) {
    this.dragAndDropHandler?.try_move_draggable_to_container(
      name,
      container_name
    );
  }
   ...
   */
  public get_dnd_container_content_size(
    container_name: string
  ): [number, number] {
  ...
  public custom_object(
    ...
    f_draw_to_svg(
      ...
      true,
      false,
      calculate_text_scale(diagram_svg)
    );
    svg.setAttribute("overflow", "visible");
    svg.setAttribute("class", classlist.join(" "));
    svg.setAttribute("id", id);

    control_svg.setAttribute(
      "viewBox",
      diagram_svg.getAttribute("viewBox") as string
    );
    control_svg.setAttribute(
      "preserveAspectRatio",
      diagram_svg.getAttribute("preserveAspectRatio") as string
    );
  ...
  private init_button() {
    if (this.diagram_outer_svg == undefined)
      throw Error("diagram_outer_svg in Interactive class is undefined");
    let diagram_svg = this.get_diagram_svg();
    let button_svg = this.get_svg_element(
  ...
  public button_toggle(
    ...
    let setter = this.buttonHandler.try_add_toggle(
      name,
      diagram_on,
      diagram_off,
      state,
  ...
  public button_click(
    ...
    this.buttonHandler.try_add_click(
      name,
      diagram,
      diagram_pressed,
      diagram,
...
function set_viewbox(
  taget: SVGSVGElement | undefined,
  source: SVGSVGElement | undefined
) {
  ...
  taget.setAttribute(
    "preserveAspectRatio",
    source.getAttribute("preserveAspectRatio") as string
  );
}
 ...
 */
export function clientPos_to_svgPos(
 ...
 */
export function get_SVGPos_from_event(
  evt: LocatorEvent,
  svgelem: SVGSVGElement
): { x: number; y: number } {
...
class LocatorHandler {
  ...
  constructor(
    public control_svg: SVGSVGElement,
    public diagram_svg: SVGSVGElement
  ) {}
  ...
  setViewBox() {
    ...
    this.control_svg.setAttribute(
      "viewBox",
      this.diagram_svg.getAttribute("viewBox") as string
    );
    this.control_svg.setAttribute(
      "preserveAspectRatio",
      this.diagram_svg.getAttribute("preserveAspectRatio") as string
    );
  }
  ...
  handleBlinking() {
    ...
    for (let i = 0; i < this.blinking_circle_outers.length; i++) {
      this.blinking_circle_outers[i].classList.remove(
        "diagramatics-locator-blink"
      );
    }
  ...
  create_locator_diagram_svg(
    ...
    f_draw_to_svg(
      this.control_svg,
      g,
      diagram.position(V2(0, 0)),
      true,
      false,
      calculate_text_scale(this.diagram_svg)
    );
  ...
  create_locator_circle_pointer_svg(
    ...
    let circle_outer = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "circle"
    );
    let circle_inner = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "circle"
    );
...
class DragAndDropHandler {
  ...
  move_validation_function:
    | ((draggable_name: string, target_name: string) => boolean)
    | null = null;
  ...
  generate_position_map(
    ...
    switch (config.type) {
      ...
      case "vertical": {
        ...
        const sizelist = content.map(
          (name) => this.draggables[name]?.diagram_size ?? [0, 0]
        );
        const size_rects = sizelist.map(([w, h]) => rectangle(w, h).mut());
        const distributed = distribute_vertical_and_align(
          size_rects,
          config.padding
        )
          .mut()
      ...
      case "horizontal": {
        ...
        const size_rects = sizelist.map(([w, h]) => rectangle(w, h).mut());
        const distributed = distribute_horizontal_and_align(
          size_rects,
          config.padding
        )
          .mut()
  ...
  get_container_content_size(container_name: string): [number, number] {
    ...
    const content_diagrams = container.content.map(
      (name) => this.draggables[name]?.diagram ?? empty()
    );
  ...
  public add_draggable_to_container(
    name: string,
    diagram: Diagram,
    container_name: string
  ) {
  ...
  public add_draggable_with_container(
    name: string,
    diagram: Diagram,
    container_diagram?: Diagram
  ) {
  ...
  register_move_validation_function(
    fun: (draggable_name: string, target_name: string) => boolean
  ) {
  ...
  public drawSvg() {
    for (let container_name in this.containers) {
      ...
      if (container_data?.svgelement == undefined) {
        const outer_g = document.createElementNS(
          "http://www.w3.org/2000/svg",
          "g"
        );
  ...
  diagram_container_from_draggable(diagram: Diagram): Diagram {
    let rect = rectangle_corner(...diagram.bounding_box()).move_origin(
      diagram.origin
    );
  ...
  private add_focus_rect(g: SVGGElement, diagram: Diagram) {
    ...
    const focus_rect = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "rect"
    );
  ...
  private move_svg_draggable_to_container(
    draggable_name: string,
    container_name: string
  ) {
  ...
  reposition_container_content(container_name: string) {
    ...
    const bbox =
      container.config.custom_region_box ?? container.diagram.bounding_box();
    const position_map = this.generate_position_map(
      bbox,
      container.config,
      container.capacity,
      container.content
    ...
    for (let i = 0; i < container.content.length; i++) {
      ...
      draggable.svgelement?.setAttribute(
        "transform",
        `translate(${pos.x},${-pos.y})`
      );
    }
  }
  remove_draggable_from_container(
    draggable_name: string,
    container_name: string
  ) {
    this.containers[container_name].content = this.containers[
      container_name
    ].content.filter((name) => name != draggable_name);
  }
  private move_draggable_to_container(
    draggable_name: string,
    container_name: string,
    ...
    this.remove_draggable_from_container(
      draggable_name,
  ...
  try_move_draggable_to_container(
    draggable_name: string,
    container_name: string,
    ignore_callback = false
  ) {
    if (this.move_validation_function) {
      const valid = this.move_validation_function(
        draggable_name,
        container_name
      );
      if (!valid) return;
    ...
    if (container.content.length + 1 <= container.capacity) {
      this.move_draggable_to_container(
        draggable_name,
        container_name,
        ignore_callback
      );
  ...
  startDrag(evt: DnDEvent) {
    ...
    this.draggedElementGhost = draggable.svgelement.cloneNode(
      true
    ) as SVGElement;
  ...
  drag(evt: DnDEvent) {
    ...
    this.draggedElementGhost.setAttribute(
      "transform",
      `translate(${coord.x},${-coord.y})`
    );
  }
...
class ButtonHandler {
  ...
  constructor(
    public button_svg: SVGSVGElement,
    public diagram_svg: SVGSVGElement
  ) {}
  ...
  try_add_toggle(
    ...
    diagram_off: Diagram,
    state: boolean,
    callback: (state: boolean, redraw?: boolean) => any
  ): setter_function_t {
